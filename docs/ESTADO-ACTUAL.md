...
- `src/central/comm_down.{h,cpp}` — recibe del DOWN por **`Serial1` (RX1 = pin 0)** y decodifica los **3 frames**: `LineStatusV2` (línea, vía `lsv2_from_frame`) + `Pose2D` (OTOS pose) + `Velocity2D` (OTOS vel) → `world_model`, con detección de pérdida por SEQ + contador `crc_errors`. **Conflicto 7/8 RESUELTO** (reasignación 2026-05-31: el motor 2 del driver U17 se quedó los pines 7/8 y el link a DOWN se movió a Serial1 / pin 0). Receiver de banco para verificar los enlaces de la CENTRAL: **`diag_central_rx_all`** (decodifica DOWN + TOP a la vez). 📊 Análisis profundo del link (protocolo/CRC, buffers, timing, recuperación ante cortes) → [`docs/firmware/ANALISIS-COMM-DOWN-CENTRAL-2026-05-31.md`](firmware/ANALISIS-COMM-DOWN-CENTRAL-2026-05-31.md).
