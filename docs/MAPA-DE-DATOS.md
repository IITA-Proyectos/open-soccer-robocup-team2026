...
| **Blobs cámara** | **11 B** (contrato v2: 9 B de datos + CRC8 + END=254; sentinel 255=no-detectado) · headers `201`=pelota / `202`=arco amarillo / `203`=arco azul | **Cámara → TOP** | frontal `Serial3` pin15 (U8) · trasera `Serial5` pin21 (U9) | ~30 Hz | script OpenMV (`cam-*-n6.py`) | `cameras`→`cameras_fusion`→`build_snapshot` | `CONTRATO-DATOS-CAMARAS.md` |
