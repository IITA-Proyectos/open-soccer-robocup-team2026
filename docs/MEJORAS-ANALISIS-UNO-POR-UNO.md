...
| C2 | **IMU — 2 BNO, 2º en Wire2 (24/25), ambos 0x28** | NEEDS-BENCH (+ cambio de código) | `sensors_imu.cpp` hoy pone los 2 BNO en `Wire` y distingue por 0x28/0x29. Para R2: leer el 2º BNO de `Wire2@0x28` (bus LPI2C4, pines 24/25, back-pad bajo el Teensy), gateado per-robot (tabla `IMU_BNO_BUS[]`/`IMU_BNO_ADDR[]` del robot-def). El BNO de `Wire2` (solo, sin ToF) es el **PRIMARIO** (más confiable, no hay contención con ToF); el de `Wire` es el **SECUNDARIO**. MISMATCH real, cambio de `.cpp`. | **TOP (R2)** — `diag_top_bno`/`diag_bno_tof` con los 2 BNO en buses separados; medir `HEADING_SIGN`. |
