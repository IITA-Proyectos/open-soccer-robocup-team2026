...
| **2 BNO en 2 buses (ROBOT2)** | NEEDS-BENCH (+ cambio de código) | Cambio en `sensors_imu.cpp` para leer un BNO por bus (Wire/**Wire2**, pines 24/25), ambos en 0x28, gateado per-robot. El BNO de `Wire2` (solo, sin ToF) es el **PRIMARIO**; el de `Wire` (con los 4 ToF) es el **SECUNDARIO**. Requiere HW ROBOT2. | **TOP (ROBOT2)** — ver gap C.2. |
...
| C2 | **IMU — 2 BNO, 2º en Wire2 (24/25), ambos 0x28** | NEEDS-BENCH (+ cambio de código) | `sensors_imu.cpp` hoy lee un solo BNO en `Wire`. Para R2 (ya cableado así en HW, ambos robots): los 2 BNO van **ambos en 0x28**, en **buses separados** — el 2º BNO en `Wire2@0x28` (bus LPI2C4, pines 24/25, back-pad bajo el Teensy), gateado per-robot (tabla `IMU_BNO_BUS[]`/`IMU_BNO_ADDR[]` del robot-def). El BNO de `Wire2` (solo, sin ToF) es el **PRIMARIO** (más confiable, no hay contención con ToF); el de `Wire` (junto a los 4 ToF) es el **SECUNDARIO**. MISMATCH real, cambio de `.cpp`. | **TOP (R2)** — `diag_top_bno`/`diag_bno_tof` con los 2 BNO en buses separados; medir `HEADING_SIGN`. |
