...
> Hoy: **545 tests / 40 suites / 0 failures** (verificado 2026-06-04 con `scripts/run-host-tests.sh`), que
> crecieron de forma trazable (246 → 262 → 324 → 354 → 545). Ver el gráfico de crecimiento en
...
> "Dos cosas: (1) **gate verde obligatorio** —los 545 tests pasan (545 / 40 / 0)— antes de cualquier merge; y (2)
...
- **Número de tests vigente** al momento de viajar: verificado **545 tests / 40 suites / 0 failures (2026-06-04 vía `scripts/run-host-tests.sh`)**. Correr el runner el día anterior y usar la cifra real del día.
