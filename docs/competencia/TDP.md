...
| `MOTOR_MIN_PWM[3]` (piso de PWM por rueda) | {70, 70, 107} | **CALIBRADO en banco 2026-06-09 (robot 2; robot 1 arranca de los mismos valores, a verificar)** — delanteras oblicuas 70 · trasera alineada 107; acompañado de impulso inicial {130,130,140}×40 ms + freno anticipado de la trasera 66 ms |
...
- **Iteración (robot 2, banco 2026-06-09):** en el strafe del robot 2 la trasera quedaba lenta y el strafe arqueaba; barrido empírico del piso trasero **42→50→70→85→95→100→105→107** → `MOTOR_MIN_PWM={70,70,107}`. **Dato físico:** el PWM no es proporcional a la velocidad y difiere por rueda — la trasera debe girar al **doble** que las delanteras (cinemática: fronts 0,5·vx, rear 1,0·vx) pero, por rodar alineada (menos fricción que las oblicuas), lo logra con **~1,5× el PWM (107 vs 70), no 2×**. Se sumaron, como estándar de todo movimiento lateral: **impulso inicial fijo {130,130,140} PWM ×40 ms** al arrancar cada rueda (las delanteras no rompían la inercia; la trasera pedía 140) y **freno anticipado de la trasera** (corte a 0 en los últimos **66 ms** del tramo, para que su inercia no desacomode el robot al frenar). Robot 1 parte de estos mismos valores (su {70,70,42} previo, banco 2026-06-08, queda como historia: si rota con 107, se baja gradualmente).
...
**Lenguajes:** C++17 (firmware, namespace `iitasoccer`, structs `__attribute__((packed))` con `static_assert` de tamaño) + MicroPython (visión OpenMV N6, `find_blobs` por color LAB). Build: PlatformIO (**83 entradas `[env:]`**: envs de producción por robot + ~57 de diagnóstico/banco/test).
...
- **App de testeo/calibración/monitoreo de banco** (`software/teensy/Soccer 2026/tools/monitor-base/`, Python stdlib): lee la **telemetría USB** (modo debug gateado, contrato JSON Lines **versionado** validado con golden cross-lenguaje) y muestra los sensores de las placas de forma **visual** —anillo de 32 + calibración asistida, vista de arquero (cross-track + OTOS), radar del campo TOP—; **corre sin el robot** (simulador + replay). Es el precursor de la telemetría en tiempo real (ver §3.10 y §4.6).
