...
|---|---|---|
| **"Easy to follow / a peer competitor understands it"** (Satisfactory = 1) | **Satisfactory (= máximo, 1 pt)** | Estructura problema→solución→cómo-replicarlo (bloques 1–5); ritmo ~145 ppm; subtítulos EN; jerga explicada; demo en pantalla de 47 suites en verde (652 tests / 47 suites / 0 failures, medido 2026-06-05 18:39 ART vía scripts/run-host-tests.sh) |
...
3. **Tiene un número real, fresco y verificable HOY.** Corrí la suite el **2026-06-05** y dio **652 tests / 47 suites / 0 failures (measured 2026-06-05 18:39 ART via scripts/run-host-tests.sh)** en segundos, 100% offline. No es una promesa: es reproducible con `bash scripts/run-host-tests.sh`.
...
| 1:26 | "Cuarenta y siete suites de tests. Seiscientos cincuenta y dos casos. Cero fallos. En segundos, sin tocar el robot." | [IMG: congelar en la línea final `Tests: 652 | Failures: 0  (Envs: 47 | OK: 47)`; zoom y resaltado] | **Número grande:** "652 tests / 47 suites / 0 failures · in seconds"<br>[FIGURE: docs/competencia/assets/fig8_test_growth.png] |
| 1:36 | *(opcional, recortable si pasa de 3:00)* "Y no es magia de una vez: la suite creció de forma trazable sesión a sesión —180, 246, 324, 403… hasta 652— cada bug que encontramos quedó como un test que nunca vuelve a pasar desapercibido." | [IMG: gráfico de barras animado del crecimiento 180→246→324→403→652 — usar `docs/competencia/assets/fig8_test_growth.png` (gen_figuras.py)] | **Gráfico:** "Tests over time: 180 → 246 → 324 → 403 → 545 → 652" |
...
1. **[CAPTURA — CRÍTICA]** Screencast REAL de `bash scripts/run-host-tests.sh` corriendo de principio a fin, con la última línea `Tests: 652 | Failures: 0  (Envs: 47 | OK: 47)` bien legible. *(Verificado el 2026-06-05 18:39 ART: la suite da exactamente eso, exit code 0.)* Grabar en una terminal con fuente grande y tema de alto contraste.
...
2. **[DIAGRAMA]** **Gráfico de barras del crecimiento de tests**: 180 → 246 → 324 → 403 → 545 → 652 (fechas: snapshots de `docs/ESTADO-ACTUAL.md`; 652 medido 2026-06-05 18:39 ART).
...
- **Legibilidad de las terminales:** fuente ≥ 18 pt, tema de alto contraste, zoom/resaltado sobre la línea `Tests: 652 | Failures: 0  (Envs: 47 | OK: 47)`. Es el momento clave: el juez tiene que LEER el número.
...
- **Datos reales ya verificados (usar verbatim):**
  - **652 tests / 47 suites / 0 failures (measured 2026-06-05 18:39 ART via scripts/run-host-tests.sh)** (exit code 0). *(NOTA: la rúbrica y otros entregables citaban un tope previo más bajo; el número subió a 652 tests / 47 suites — está verificado en esta sesión y es la cifra a publicar.)*
  - Crecimiento trazable: **180 → 246 → 262 → 324 → 354 → 403 → 545 → 652** (snapshots en `docs/ESTADO-ACTUAL.md`).
...
| 9 | **[VERIFICAR al cerrar]** re-correr `scripts/run-host-tests.sh` el día de grabar para confirmar que la cifra sigue siendo **652/47/0** (o actualizar el número sobreimpreso si cambió por nuevos tests) | Dato | Bloque 3 (1:26, 1:36) |
| 10 | **[OPCIONAL]** confirmar fechas exactas de cada snapshot del gráfico de crecimiento (180→…→652) si se quiere fechar las barras | Dato | Bloque 3 (1:36, narración opcional) |
