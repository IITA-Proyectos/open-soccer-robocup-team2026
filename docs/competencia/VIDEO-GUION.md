...
|---|---|---|
| **"Easy to follow / a peer competitor understands it"** (Satisfactory = 1) | **Satisfactory (= máximo, 1 pt)** | Estructura problema→solución→cómo-replicarlo (bloques 1–5); ritmo ~145 ppm; subtítulos EN; jerga explicada; demo en pantalla de 47 suites en verde (658 tests / 47 suites / 0 failures, medido 2026-06-05 19:50 ART vía scripts/run-host-tests.sh) |
...
3. **Tiene un número real, fresco y verificable HOY.** Corrí la suite el **2026-06-05** y dio **658 tests / 47 suites / 0 failures (measured 2026-06-05 19:50 ART via scripts/run-host-tests.sh)** en segundos, 100% offline. No es una promesa: es reproducible con `bash scripts/run-host-tests.sh`.
...
| 1:18 | "Y esto es lo que vemos al correr la suite, hoy mismo:" | [IMG: **screencast REAL** de la terminal escupiendo línea por línea `PASS test_proto`, `PASS test_kinematics`, `PASS test_localization`… acelerado] | **Texto (aparece al cierre):** "live run · 2026-06-05" |
| 1:26 | "Cuarenta y siete suites de tests. Seiscientos cincuenta y ocho casos. Cero fallos. En segundos, sin tocar el robot." | [IMG: congelar en la línea final `Tests: 658 | Failures: 0  (Envs: 47 | OK: 47)`; zoom y resaltado] | **Número grande:** "658 tests / 47 suites / 0 failures · in seconds"<br>[FIGURE: docs/competencia/assets/fig8_test_growth.png] |
| 1:36 | *(opcional, recortable si pasa de 3:00)* "Y no es magia de una vez: la suite creció de forma trazable sesión a sesión —180, 246, 324, 403… hasta 658— cada bug que encontramos quedó como un test que nunca vuelve a pasar desapercibido." | [IMG: gráfico de barras animado del crecimiento 180→246→324→403→658 — usar `docs/competencia/assets/fig8_test_growth.png` (gen_figuras.py)] | **Gráfico:** "Tests over time: 180 → 246 → 324 → 403 → 545 → 658" |
...
1. **[CAPTURA — CRÍTICA]** Screencast REAL de `bash scripts/run-host-tests.sh` corriendo de principio a fin, con la última línea `Tests: 658 | Failures: 0  (Envs: 47 | OK: 47)` bien legible. *(Verificado el 2026-06-05 18:39 ART: la suite da exactamente eso, exit code 0.)* Grabar en una terminal con fuente grande y tema de alto contraste.
2. **[CAPTURA]** El comando, precedido por `cd "software/teensy/Soccer 2026"`, luego `g++ -std=gnu++17 -I src/shared -I lib/Unity/src lib/Unity/src/unity.c src/shared/*.cpp test/test_X/test_main.cpp -o ...` (de `scripts/run-host-tests.sh`: flags en la línea 43, compilación en la línea 80) en pantalla, resaltando que los includes reales son `-I src/shared -I lib/Unity/src` (NO `-I src/down`, NO `lib/Unity`) y que linkea `src/shared/*.cpp`.
...
2. **[DIAGRAMA]** **Gráfico de barras del crecimiento de tests**: 180 → 246 → 324 → 403 → 545 → 658 (fechas: snapshots de `docs/ESTADO-ACTUAL.md`; 658 medido 2026-06-05 19:50 ART).
...
- **Legibilidad de las terminales:** fuente ≥ 18 pt, tema de alto contraste, zoom/resaltado sobre la línea `Tests: 658 | Failures: 0  (Envs: 47 | OK: 47)`. Es el momento clave: el juez tiene que LEER el número.
...
- **Datos reales ya verificados (usar verbatim):**
  - **658 tests / 47 suites / 0 failures (measured 2026-06-05 19:50 ART via scripts/run-host-tests.sh)** (exit code 0). *(NOTA: la rúbrica y otros entregables citaban un tope previo más bajo; el número subió a 658 tests / 47 suites — está verificado en esta sesión y es la cifra a publicar.)*
  - Crecimiento trazable: **180 → 246 → 262 → 324 → 354 → 403 → 545 → 658** (snapshots en `docs/ESTADO-ACTUAL.md`).
...
| 2 | ✅ RESUELTO 2026-06-05: Roboliga Argentina 2025 (final nacional, UAI); lower-third = "Salta, Argentina" | Identificación | Lower-third 0:08 |
...
| 9 | **[VERIFICAR al cerrar]** re-correr `scripts/run-host-tests.sh` el día de grabar para confirmar que la cifra sigue siendo **658/47/0** (o actualizar el número sobreimpreso si cambió por nuevos tests) | Dato | Bloque 3 (1:26, 1:36) |
| 10 | **[OPCIONAL]** confirmar fechas exactas de cada snapshot del gráfico de crecimiento (180→…→658) si se quiere fechar las barras | Dato | Bloque 3 (1:36, narración opcional) |
