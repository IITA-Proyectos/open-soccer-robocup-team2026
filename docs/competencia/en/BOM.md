...
| Component | Part number / model | Qty | Source / supplier | New / Reused | Kit / Custom | Unit cost | Total cost |
|---|---|---|---|---|---|---|---|
| Traction motor | **DC brushed "TT" type motor** (5 V common, KIWI base 3 wheels at 120°; model/torque/RPM exact **[to confirm]**) | 3 | generic / Pololu | **[New/Reused?]** — 2025 used "TT motors" | Module (kit) | **USD 23.95/u** (max ref.: Pololu 30:1 HP #1093; the generic TT is ~USD 3) | **USD 71.85** |
...
| Omnidirectional wheel | **38 mm omni wheel** (Nexus 14184 + 3 mm axle hub; Ø/material/rollers exact **[to confirm]**) | 3 | Nexus / generic | **[New/Reused?]** | Module (kit) | **USD 6.50/u** (omni ~4.00 + hub ~2.50) | **USD 19.50** |
...
|---|---|---|---|---|---|---|---|
| Battery | **LiPo 2S · 7.4 V nominal · 6800 mAh** (8.4 V full / ~6.0 V cutoff; energy **≈ 50 Wh** = 6.8 Ah × 7.4 V; Deans connector; C-rating/brand/weight **[to confirm]**, ref. 50C) | 1 | generic (RC) | New | Module (kit) | **USD 42.99** (max ref.: Gens Ace 7.4 V 6800 mAh 50C 2S) | **USD 42.99** |
...
| Tactile buttons | **TS-1088-AR02016** (SW-SMD; CONNECT/PROG = user button + GPIO9 boot strap) | 2 | generic | New | SMD on COMM PCB | **USD 0.10** (max ref.) | **USD 0.20** |
| **Subtotal §1.7** | | | | | | | **USD 2.70** |

> ⚠️ **Note COMM:** the referee starts/stops by **LEVEL of voltage on OUT_1/OUT_2** (3.3 V=GO, 0 V=STOP), **NOT by UART** — the C6 firmware receives the command via BLE from the referee app and translates it to level. The UART `RX_OUT/TX_OUT` (via TXS0102) is passive hardware that the official firmware v0.91 does not yet use. The OLED display of the RCJ module is **external** (not populated on this board, goes via I²C on U4).
...
| **2× BNO055** (redundancy of heading) | 1 IMU | "two chips of ~USD 35 each; much superior reliability" | 2 IMU (today 1 healthy; see gap) |
...
| §1.7 Specific ICs COMM board | **USD 2.70** |
...
| 3 | ⚙️ **[COSTS] reference values LOADED** (2026-06-07, a single value = the highest per item, §1.1–§1.7 with Total cost per row). Pending: **actual prices/invoices** from the team (vs international reference) + confirm Zircon separately and actual motor | Cost |
...
| 6 | **[SPEC wheel]** Ø/material/no. of rollers/source (printed or purchased) of the omni wheel 2026 | Mechanical |
...
| 12 | **[TEMPLATE]** check if an official RCJ BOM template exists and transcribe | Format |
