...
- **Instant board-free verification**: `bash scripts/run-host-tests.sh` → runs the 658 host tests in seconds (658 tests / 47 suites / 0 failures, measured 2026-06-05 19:50 ART). **Showing this to the judge is a winner**: "before uploading to the robot, we validate it on the PC". **Honest caveat:** the host runner compiles the **pure modules** (shared + down); the central/top tests use Arduino and compile **on-target** on the board.
...
> **658 tests / 47 suites / 0 failures** (measured 2026-06-05 19:50 ART with `scripts/run-host-tests.sh`), which grew traceably
> (246 → 262 → 324 → 354 → 545 → 658). See the growth chart at `docs/competencia/assets/fig8_test_growth.png`. **Honest
...
> "Two things: (1) a **mandatory green gate** —all 658 tests pass (658 / 47 / 0)— before any merge; and (2) a
...
- **Current test count** at travel time: verified **658 tests / 47 suites / 0 failures (2026-06-05 18:39 ART via `scripts/run-host-tests.sh`)**. Run the runner the day before and use the real figure of the day.
