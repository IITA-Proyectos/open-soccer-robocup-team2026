...
> **(1) Show & Tell**, **(2) Teamwork-Task** (judges give a programming task on the spot) and **(3) Questions** (bank of questions by category). It is judged with 3 criteria, each on a scale of 0/1/3/5. **We aim for Excellent (5) in all three.**
>
> **Gold rule of the interview (what the judge looks for):** that **EVERYONE** contributes, that there is **real technical fluency** (not reciting) and **innovative approaches**. No one monopolizes, no one is silent.
>
> **Language note:** the interview is conducted in **English** (the competition language). The 🇬🇧 marker indicates phrases that should be translated and practiced aloud before Incheon; the English version of this sheet lives in `docs/competencia/en/ENTREVISTA-PREP.md`.
...
|---|---|---|
| **Teamwork & Communication** | Fluid collaboration, members support each other, **clear roles**, **EVERYONE contributes** | §2 (role distribution) + §1 (everyone speaks in the Show&Tell) + §5 (protocol for passing the floor) |
...
| 85-90 s | both | "Everything is **open-source with MIT license** on GitHub, documented so that another team can replicate it. Where do you want to start?" |
...
| **Development & Documentation** | **shared** | Engineering journal + TRUTH-SOURCES + traceable tests | "How do you track progress?" / "How do you know something is working?" |

**Team rule for the interview (say this if a judge asks "who did what?"):**
> "We work by areas but with **a shared repository**: each change comes with its entry in the **engineering journal** and, if it touches a critical data point, the **TRUTH-SOURCES** table is updated in the same commit. This way, no one depends on the memory of the other."

---

## §3 — Live Teamwork-Task (judges give the task) → aims for Task Execution (Excellent: "efficient + innovative")

> The judge gives a programming task on the spot (e.g., "make the robot turn until it sees the ball and approach", "stop when crossing the line", "patrol side to side"). **Do not improvise the organization: follow this protocol.** What earns Excellent is that a **method** is visible (not chaos) and a **unique approach** (reusing our pure modules + flashing quickly).
...
3. **Talk while doing**: narrate what is being touched ("I am going to modify `strategy.cpp` in APPROACH state, increasing the approach speed"). The judge scores based on what they understand.
...
- "We are going to solve it **by reusing a pure module that is already tested** instead of writing new logic blindly" → efficiency + engineering criteria.
- "We add a **fallback**: if the new data is not available, it does exactly what it did before" → this is our *fallback byte-identical* technique, zero regression. Very sellable.
...
> Each answer: **concrete data + why + (if applicable) a real iteration**. The judge rewards us giving **numbers** and telling **a data-driven decision**, not generalities. 🇬🇧 = rehearse in English.
...
> "It is the standard pattern of mobile robotics (used by Middle Size League teams like CAMBADA). And we started from **what already worked for us**: the central board is the **Zircon** with which we won the National 2025; we added the perception and ground boards around it without replacing it."

### Electrical (component selection, troubleshooting)

**Q: How did you choose the components?**
> "COTS and cheap criteria, purchasable at LCSC: **Teensy 4.0/4.1** (Cortex-M7 at 600 MHz) as brains, **VL53L7CX** multi-zone ToF for distance, **BNO055** for heading, **OTOS from SparkFun** for optical odometry, and **OpenMV N6** cameras. We power with **LiPo 2S (7.4 V)** → protection with Schottky diodes → 2 buck regulators **MP1584** per board (5 V logic, 3.3 V sensors). We characterized the drift of the OTOS on the bench — see the error graph in `docs/competencia/assets/fig9_otos_error.png`."

**Q (troubleshooting): Tell me about an electrical problem you had and how you solved it.** 💡
> "The **4 ToF sensors** all start on the same I²C address (0x29) and clash on a shared bus. Investigating the schematic, we discovered that the pins for enumerating them **were not routed** on the PCB. Enzo made a **bodge**: he wired the control pin of each ToF to a GPIO of the Teensy. And we learned something key: their addresses **persist while they have 3.3 V**, so we need to **power-cycle**, not reset. After that, the 4 ToF enumerate to 0x2A-0x2D and **2D localization by trilateration** was unlocked."

**Q: Why is one motor inverted in the code?**
> "The driver for motor 2 (U17) has the inputs crossed by hardware on the Zircon shield. Instead of rewiring, we corrected it in **one place in the firmware**: `MOTOR_INVERT={+1,-1,+1}`. We **validated it on the bench** by rotating each motor separately with `diag_central_motors`."
...
> "The **structural base plate is directly the PCB at the bottom** (≈175 × 166 mm, rounded plate contour, M3 mounting). The 3 boards stack like floors. `[GAP: confirm materials of the rest of the chassis, height between floors and printed parts 2026 — recorded in gaps]`."
...
> "Patrols the goal and, when the ball approaches, **intercepts by anticipating**: instead of going to the current X of the ball, it goes to the predicted X = position + speed × 0.2 s, capped at 400 mm. The speed of the ball is calculated on the vision board by finite differences with an EMA filter. If the ball is stationary, the lead is 0 and it behaves like a normal goalkeeper — **automatic fallback**."
...
> "Ring of **32 line sensors** on the ground board. When imminent exit is detected, there is a **direct emergency bus** from that board to the central one (1 UART hop) to stop in **<15 ms** — at 1 m/s that is 15 mm. If we were to stop passing through the vision board (2 UARTs), we would have already crossed."
...
> "The vision board **fuses** 2 cameras + IMU + 4 ToF and builds a *WorldSnapshot* of 31 bytes: own pose, ball (position **and speed**), goals, closest obstacle, and referee command. It sends it to the central board at 100 Hz. The central board only consumes that summary — it does not touch raw sensors."

**Q: How do the boards communicate? And what happens if a message is lost?**
> "UART with a **custom protocol**: `[START 0xAA | LEN | TYPE | SEQ | PAYLOAD | CRC-16 | END 0x55]`. The **CRC** detects corruption and the **SEQ** detects lost packets. The decoder is a **byte-by-byte state machine that resynchronizes itself**: a garbage byte does not contaminate the next frame. And there are **watchdogs**: if a stream does not arrive in 500 ms, the central goes into safe mode."

**Q: How do you debug? Tell me about a real software bug.** 💡
> "We have ~40 diagnostic sketches that reuse the production parsers. A real bug: the central became **blind to the line** because it was decoding the old format (5 bytes) and **discarding** the new frames (16 bytes). It was invisible in telemetry — it looked the same as a loose cable. We caught it with a **harness in g++ offline** of the real chain encode→decode→interpret, migrated to the new format and covered it with tests. **Team rule**: when a failure occurs, first grep the journal to see if it has happened before."

**Q: Coordination between the 2 robots?**
> "Through **ESP-NOW** via the communication board (ESP32-C6). The partner shares pose and if it sees the ball, and that enters the WorldSnapshot. `[GAP: confirm status of partner coordination in the bench]`."

### Development & Documentation (inspiration, tracking, testing)

**Q: How do you track progress?**
> "With three live indices: **CURRENT-STATE** (robot state on 1 page, mandatory reading), **TRUTH-SOURCES** (a canonical doc per topic; whoever creates or surpasses a doc updates the table in the **same commit**) and a chronological **engineering journal**. Plus **self-contained packs** by subsystem for onboarding."

**Q: How do you test? (the star) 💡**
> "We separate the **decision logic into pure C++ modules** —without Arduino— and test them on the PC with g++. Today: **658 tests / 47 suites / 0 failures** (measured 2026-06-05 19:50 ART with `scripts/run-host-tests.sh`), which have grown traceably (246 → 262 → 324 → 354 → 545 → 658). See the growth graph in `docs/competencia/assets/fig8_test_growth.png`. **Honest clarification:** the host runner compiles the **pure modules** (shared + down); the central/top tests use Arduino and are compiled **on-target**. It originated from a real problem: the **antivirus blocked PlatformIO**, so we vendored the testing framework and wrote a runner in g++ that **bypasses the antivirus and runs without internet**. This allows us to verify embedded firmware **without having the board in hand**."

**Q: How do you ensure that a change does not break what was working?**
> "Two things: (1) **mandatory green gate** —the 658 tests pass (658 / 47 / 0)— before any merge; and (2) **byte-identical fallback**: each new feature, if the data is not available, produces **exactly** the previous command. We verify this with a test that compares the output with and without the data. This way a feature 'sleeps' until the data flows and **never introduces a regression**."
...
| I don't know the answer | "Honestly, I have that half-done, we validated it on the bench; what I do know is..." + redirect to what I do master | Make up / stay silent |
| My partner gets stuck | Support them: "and a detail that helps here is..." | Leave them alone or correct them publicly with brusqueness |
| The judge asks something we haven't validated on the bench | **Be honest**: "it is implemented and tested on host, it lacks validation on the field" | Say it works if we haven't tested it |
...
- **Vision not recalibrated for Incheon:** "The vision code is solid and tested; what is missing is **bench calibration** (LAB + homography) for the venue lighting. We have the kit ready to recalibrate in <5 min." (This is our real blocker #1 — do not hide it.)
- **Tentative kinematics:** "The angles and wheel radius are marked as **tentative** in the code because we lacked measuring the assembled robot; the module is pure and tested, we just need to load the real constants."
...
- **Heading freezing:** "We detected that the IMU and ToF compete on the I²C bus and the heading froze; we mitigated by lowering the bus to 100 kHz and reading the IMU at 20 Hz. The underlying fix (IMU on a separate bus) is noted."
...
- Officially registered `IITA Low Battery Messi` in RoboCup Junior for Incheon (confirmed 2026-06-05; "IITA - Open Soccer RoboCup Team 2026" is the internal descriptor of the repo).
...
- **Status of partner coordination (ESP-NOW) on the bench** — to confidently answer the "robot-robot" question.
...
- **Legal name of IITA** (✅ resolved 2026-06-05): **IITA = Instituto de Innovación y Tecnología Aplicada** / Fundación Innovar. Unified in all docs.
