...
The **zone titles are written for a JUDGE to find each rubric criterion at a glance**:

| Rubric Criterion (Poster, 5 pts) | Where does the judge find it? | Target Level |
|---|---|---|
| **Abstract** | Zone B — "ABSTRACT" | Excellent |
| **Method / Robot Production / Design** | Zone C, D, E, F — "METHOD & DESIGN" | Excellent |
| **Data / Results / Discussion** | Zone G, H — "DATA, RESULTS & DISCUSSION" | Excellent |
...
# ZONE B — ABSTRACT
*(Left column, top — mandatory element "Abstract". Aims for **Excellent**: summarizes EVERY critical component with scientific language and a clear intention to share actionable knowledge. DOES NOT repeat the details of the other zones: it synthesizes.)*
...
> ## OUR JOURNEY
> - **Dec-2025 — national champions with a MUCH more basic robot:** above **a single camera** (without ToF or ultrasonic), below **only 3 light sensors**, all on the Zircon board… and **still we won the 1st national competition of RoboCupJunior Soccer in Argentina**.
> - **2026 — the leap:** same champion brain (Zircon), much more perception. Above: **2 cameras + IMU + 4 ToF + ultrasonic**. Below: **ring of 32 sensors + 2 OTOS**. Redesign to **3 boards** adding perception (TOP) and floor (DOWN) — *continuity, not discard*.
...
`[BEFORE/AFTER PHOTO — Fig.1a: champion robot of the National 2025 — basic version: 1 camera, 3 light sensors · Fig.1b: robot 2026 — 2 cameras + IMU + 4 ToF + ultrasonic above, ring of 32 sensors + 2 OTOS below. Same Zircon brain.]`
`[PHOTO: IITA team with the robot/trophy at the National 2025 (UAI) — label Fig.1]`

> **Team roles:**
> | Role | Member | Technical Contribution |
> |---|---|---|
> | Mentor (not traveling) | Gustavo Viollaz | Coordination, lab sessions, integration of 3 boards |
...
> - **How, not just what:** living documents **TRUTH-SOURCES** (a canonical doc per topic), **DATA-MAP** (each message: type/size/pin/frequency/who fills and consumes it) and **engineering journal** with each iteration.
>
...
> **Languages:** **C++17** (firmware, `namespace iitasoccer`, packed structs with `static_assert` for size) + **MicroPython** (OpenMV N6 vision, color detection in LAB space).
...
> | ToF VL53L7CX (8×8 zones) | 4 | Distance to walls → **2D localization** |
...
> | MCU CENTRAL | Teensy 4.1 (on Zircon PCB) | 1 | **Reused** (2025 champion) | **USD 31.50** (int. ref.) |
...
> | Zircon Rev v15 PCB (CENTRAL) | Robomov Zircon Rev v15 (COTS) | 1 | **Reused** | **~USD 200** (pending loose Robomov price — kit USD 529) |
...
> | LiPo 2S 7.4 V battery | LiPo 2S (mAh/C to confirm) | 1–2 | New | **~USD 10–25** (specs pending) |
> | 3 DC motors + 3 omni wheels | Motor "TT" + omni wheel KIWI | 3+3 | New | **motor pending** (TT ~USD 3 vs Pololu HP USD 23.95) |
> | **TOTAL per robot** | — | — | — | **≈ USD 1,000 new · ≈ USD 770 reusing CENTRAL** (int. ref.; 2 robots ≈ USD 1,800–2,000) |
>
> **Development time:** redesign 2026 ≈ **8 weeks** of intensive engineering (May–June), based on the 2025 champion robot; total effort ≈ **4 months** (Feb–Jun 2026) traceable in `journal/`. Test suite growing **180 → 246 → 262 → 324 → 354 → 545 → 658** (measured on 2026-06-05 19:50 ART with `scripts/run-host-tests.sh`; see Fig.8).
>
...
> **Gap note (record):** costs = **international reference price** (USD, verified 2026-06-05; single source `BOM.md` §3). **Pending from the team (small):** price of the **loose Zircon** (Robomov lists the kit at USD 529), what **motor** they use (TT ~USD 3 vs Pololu HP USD 23.95), the **ARS/USD exchange rate** of the day, and the **hours** of development. The local *landed* cost is **higher** due to Argentine import restrictions.
...
> Each row is a **real** lab iteration: we measured → evaluated → **modified the source at a single point** → re-verified with the host suite.

> | # | Observed problem | Measured data (lab) | Applied modification |
> |---|---|---|---|
> | 1 | **The 4 ToF clash on I²C** (all start at 0x29); the PCB rev 1.0 did not route XSHUT (8 *No-Connect*) | Forensic schematic: 0 nets XSHUT. After *bodge* + **power-cycle**: 4 LP on pins {9,10,11,12} enumerate to **0x2A–0x2D** | Migrate the 4 ToF to the single `Wire` bus; free `Wire1` for DOWN; **unlocks 2D localization** |
> | 2 | **BNO heading freezes** in production (yaw stuck at −108.3°) although the snapshot arrives healthy | BNO + ToF cannot coexist at 400 kHz; at **100 kHz + BNO @20 Hz** the heading survives (band-aid) | I²C at 100 kHz + reading the BNO at 20 Hz; design decision: the goalkeeper's heading-hold uses the **OTOS** (local), not the BNO |
> | 3 | **OTOS odometry** reports garbage on A4 sheet | A4 with sheet: 28.6/300 mm (9.5%); A4 without sheet: 0.3 mm; **corrugated cardboard: 280.4/300 mm = 6.5% error** (< 8% tol.) | Remove sheet + require textured floor → **10× improvement**; the green RCJ field already meets this |
...
> | 5 | **Motor 2 (U17) spins inverted** due to HW (INA/INB crossed) | Lab wheel-by-wheel (María/Elías) | `MOTOR_INVERT = {+1, −1, +1}` applied in **one place** |
...
> **Discussion:** major modifications **#1, #4, and #8 unlocked entire capabilities** (2D localization, referee homologation, edge braking). The parallel audit (20 subsystems, adversarial review) closed with **15/20 "solid", 0 critical**.

---

# ZONE H — DATA (2/2): REPEATABLE TESTING METHODS + GRAPHS
*(Right column, lower half — mandatory element "repeatable testing methods". Aims for **Excellent**: the method described for **others to repeat**, with graphs/tables.)*
...
> **Result: 658 tests / 47 suites / 0 failures** (measured on 2026-06-05 19:50 ART with `scripts/run-host-tests.sh`). **Living figure —** we re-measure it every session and it keeps rising; that's why it goes with date and time, never out of sync. It avoids the antivirus that blocked PlatformIO.
>
...
`[PHOTO: the bodge of the 4 LP of ToF wired to GPIO 9/10/11/12 (strong visual story) — Fig.11]`
...
- [x] ✅ COMPLETE Identity 2026-06-05: team IITA Low Battery Messi · region Roboliga Argentina 2025 (UAI) · roster (María Virginia Viollaz / Elías Cordero + coach Enzo Juárez Velázquez / mentor Cecilia Budeguer).
- [ ] Costs of **international reference already loaded** (≈USD 1,000 new / 770 reusing CENTRAL). Pending from the team: **loose Zircon price, motor, ARS/USD exchange rate and hours**.
