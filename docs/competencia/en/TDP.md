...
**Data:** **652 tests / 47 suites / 0 failures (measured 2026-06-05 18:39 ART via scripts/run-host-tests.sh)**. Session-to-session growth, fully traceable: 180 → 246 → 262 → 324 → 354 → 403 → 470 → 545 → 652. See **Fig. 8 — host-native test coverage growth** (`docs/competencia/assets/fig8_test_growth.png`, generated by `gen_figuras.py`). **Living figure:** we run the suite every session, so the number keeps growing — it will be higher by Incheon. That is why we publish it with the **exact measurement date and time** (re-measure on recording/printing day; the chart regenerates with `gen_figuras.py`).
...
1. **Verifying embedded firmware without the board is possible** and it changes iteration speed (pure logic + g++ host = 652 tests in seconds).
...
- **3-board firmware** (`software/teensy/Soccer 2026/`): C++17, pure modules + glue, 57 PlatformIO envs, **652 tests / 47 suites / 0 failures (measured 2026-06-05 18:39 ART via scripts/run-host-tests.sh)**.
...
- [RESOLVED] Test count at closing: **652 tests / 47 suites / 0 failures (measured 2026-06-05 18:39 ART via scripts/run-host-tests.sh)** — used consistently throughout the TDP.
...
- [PHOTO] The suite of 652 host tests green + bench diag decoding the WorldSnapshot. (Data figures already available: `docs/competencia/assets/fig8_test_growth.png` and `fig9_otos_error.png`, generated by `gen_figuras.py`.)
