...
**Data:** **624 tests / 44 suites / 0 failures (verified 2026-06-04 via scripts/run-host-tests.sh)**. Session-to-session growth, fully traceable: 180 → 246 → 262 → 324 → 354 → 403 → 470 → 545. See **Fig. 8 — host-native test coverage growth** (`docs/competencia/assets/fig8_test_growth.png`, generated by `gen_figuras.py`).
...
- **3-board firmware** (`software/teensy/Soccer 2026/`): C++17, pure modules + glue, 50 PlatformIO envs, **624 tests / 44 suites / 0 failures (verified 2026-06-04 via scripts/run-host-tests.sh)**.
