...
We use AI (mainly **Claude**, operated by us via *Claude Code* and **MCP** servers) as an **engineering tool that accelerates work**, exactly like a calculator, a CAD environment, a compiler, or a linter accelerates work — **but without replacing human decisions**. Our principle is one and it is non-negotiable:

> **AI accelerates; competitors (18 years old) decide, validate in real hardware, and are solely responsible for what goes on the robot.**
...
In our flow, AI **proposes**; the human **disposes**. Specifically, no artifact reaches the robot or deliverable without going through **at least one** of these human/objective controls:
...
All of the above (legitimacy, *human-in-the-loop*, honest limits) describes **how** we use AI. This subsection states **why**. And the answer is one, and it is the heart of our project:
...
- **VIBE PCB Design** (schematic/routing assisted via MCP, §4.1) compresses **electronic design**: the most tedious and error-prone —mapping pins, generating pinouts, reviewing netlists— no longer takes days.
...
A key feature of our orchestration: when we launch **several AI agents in parallel**, each is the **unique owner of a file** (to avoid conflicts) and there is a **central verification** that runs the gate before accepting anything. It is the same principle of *ownership* and *CI* from professional software development.
...
| **Fallback byte-identical + feature flags** | Each new capability enters as a **pure gated module** (`#ifdef`), off by default, with a fallback **byte-identical** to the previous binary. | The competition binary **does not change** until we validate. An idea from AI cannot "slip" into the robot without approval. |
...
| **Ownership by file** | In parallel work, a single agent (or person) edits each file. | Avoids conflicts and keeps responsibility clear. |
...
**Governance conclusion:** in this project, *accepting* an AI contribution **costs more human effort** than generating it. That asymmetry is intentional: it ensures that we understand and are accountable for everything we deliver.
...
**What we do.** We design the robot's PCBs (own **TOP** and **DOWN** boards; **COMM**, a fork of the official RCJ module) in **EasyEDA**, where an AI agent proposes and edits the **schematic and routing** through an **MCP server** that exposes EasyEDA as a programmable tool. The human **validates each change** of the schematic/PCB.
...
**Honest state.** This front is **just starting** — we declare it as such. The final mechanics, 3D printing tolerances, assembly, and physical validation are human. We document it as an **emerging line** of our flow, not as something mature.

### 4.3 VIBE Coding — AI-assisted C++ firmware

**What we do.** The firmware for the **3 Teensy boards** (TOP/CENTRAL/DOWN) plus the **COMM** is developed with AI assistance, but under a **specifically designed architecture to make AI safe and verifiable**:
...
**Why it is justified.** AI writes code quickly but can make mistakes; our architecture turns that risk into something **detectable and bounded**: if the logic is wrong, **a host test catches it in seconds**; if something was not validated, **it is off by a flag**. AI gives us speed; the architecture gives us safety.
...
- **Adversarial verification**: when an agent claims to have fixed/found something, another agent (or a *mutation test*) **tries to refute it** — for example, we intentionally altered an offset and confirmed that the golden test **fails**, proving that the test does not pass "in empty."
- **Coverage as a net against regressions**: every bug we find becomes a test that "never goes unnoticed again."

**Why it is justified.** Testing is **exactly** the counterbalance to the risk of AI. The more assisted code, **more** automated tests we want. AI makes cheap what was previously expensive (writing many tests), and that **elevates** quality, not lowers it.
...
**Why it is justified.** Documenting well is part of the score (TDP, poster) and **replicability** (that another team can copy our methodology). AI makes documenting stop being the bottleneck and allows us to document **at the level of detail that the rubric rewards**.
...
**How, in detail.**
- **Systematic audits**: we ran reliability audits like "champion coach" (dozens of findings, verified adversarially and classified by severity) that uncovered latent bugs and debt — each finding verified against the real code before acceptance.
- **Bench failure diagnosis**: AI helped us understand real and subtle failure modes, for example: the **yaw of the BNO freezing** due to I²C contention with the ToF at a certain clock; the **brownout of the 3.3 V rail** of the OTOS being confused with "another chip" due to a rare I²C address; the **arbitration by GPIO level** (not UART). In all, AI proposed the hypothesis and **the bench confirmed or refuted it**.
...
- **Does not decide what goes on the robot.** That is decided by the team, and remains behind flags and the gate.
- **Does not validate hardware.** Motors, edge braking, camera calibration, regulator set-points, wheel angles: all are measured/validated on the bench, by people.
...
- **Is not infallible.** That is why all of §3 (governance) exists: we assume that AI can make mistakes and build the process to **catch** those errors.
...
- **The design, engineering decisions, hardware validation, and game strategy are the team's.**
- **AI was used as an acceleration tool** in the seven fronts described, always with **human review and objective verification**.
