...
> ⚠️ **WORKING VERSION IN SPANISH.** For the submission/judges of RoboCupJunior it is translated into English (`en/USO-DE-IA.md`). This document is deliberately extensive: it is meant to thoroughly answer if a judge asks "how did you use AI?".
...
> **AI accelerates; competitors (18 years) decide, validate in real hardware, and are the only ones responsible for what goes onto the robot.**
...
**The central purpose of all this is one: to accelerate times** — compress the entire engineering cycle, from **concept** to **robot walking**. Our stated goal is to go through the **seven stages** (concept → PCB → 3D → assembly → programming → documentation → testing) in **30 days** with the materials on hand, because each VIBE front removes the bottleneck from a stage. We develop this in detail in §1.4, with the honest caveat that this year the cycle was slower — but due to the importation of materials in Argentina, not due to the method.
...
- **Legitimate (ours):** using AI to **accelerate tasks we understand**, **reviewing each output**, **validating against reality** (bench/tests), **understanding the result well enough to defend and modify it**, and **openly declaring** where it was used.
...
All of the above (legitimacy, the *human-in-the-loop*, honest limits) describes **how** we use AI. This subsection states **why**. And the answer is one, and it is the heart of our project:

> **We use AI to ACCELERATE TIMES: compress the entire engineering cycle, from the CONCEPT to the ROBOT WALKING.**
...
2. **Exploration/proposal from AI** (sometimes several agents in parallel, each owning a part).
...
A key feature of our orchestration: when we launch **multiple AI agents in parallel**, each one is **the sole owner of a file** (so they do not overwrite each other) and there is a **central verification** that runs the gate before accepting anything. It is the same principle of *ownership* and *CI* from professional software development.
...
- AI is especially useful for **tedious and error-prone human tasks**: mapping the *scrambling* of pins from the line ring multiplexers, generating the complete pinout table from the JSON of the schematic/PCB (with a reproducible script), reviewing netlists, and detecting unrouted nets.
...
**Why it is justified.** AI writes code quickly but can make mistakes; our architecture turns that risk into something **detectable and contained**: if the logic is wrong, **a host test catches it in seconds**; if something was not validated, **it is off by a flag**. AI gives us speed; the architecture gives us security.
...
- **Adversarial verification**: when an agent claims to have fixed/found something, another agent (or a *mutation test*) **tries to refute it** — for example, we deliberately altered an offset and confirmed that the golden test **fails**, proving that the test does not pass "in empty."
...
**What we do.** We use AI to **diagnose** problems: reading traces, cross-referencing symptoms with code, generating hypotheses, and proposing minimal and surgical fixes — always under verification.
...
- is **robust to lighting changes** (the problem that affects us the most today),
...
**Why it is justified.** It is the coherent evolution of our vision: moving from color heuristics to **learned perception**, leveraging hardware we already have (the NPU of the N6). And it fits with the project's philosophy: a new capability, validated on the bench, with fallback.
...
| **Host-native test suite** (`g++` + Unity, offline) | Objective referee of logic correctness | Automated (green gate mandatory) |
| **Multi-agent orchestration** (unique-owner-per-file + central verification) | Parallelize large tasks with security | Team (central verification) |
...
- *"Did AI make the robot for you?"* → No. AI accelerated tasks we understand; we decide, validate on the bench, and are responsible for everything. The proof: we can explain and modify any part, and everything is tied to tests/bench.
