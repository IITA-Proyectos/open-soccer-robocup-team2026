...
| `MOTOR_MIN_PWM[3]` | `config_central.h` (bloques R1/R2 del `#if`) | R2 `{70, 70, 107}` (**VALIDADO banco 2026-06-09**) · R1 `{70, 70, 107}` (**A-VERIFICAR-EN-BANCO**, antes `{70,70,42}`) | **PER-ROBOT (HW)** | `#if ROBOT1/ROBOT2` | Piso de PWM **POR RUEDA** (reemplazó el viejo escalar único y el efímero `MOTOR_GAIN`). ✅ **R2 `{70,70,107}` VALIDADO en banco (Gustavo 2026-06-09, `diag_central_strafe_robot2_kick`)**: la trasera se barrió 42→50→70→85→95→100→105→**107**. Física: el PWM NO es proporcional a la velocidad y es DISTINTO por rueda — en el strafe la trasera debe girar al DOBLE que las delanteras (fronts 0.5·vx, rear 1.0·vx) pero por ir ALINEADA (menos fricción que las oblicuas a 60°) lo logra con ~1.5× el PWM (107 vs 70), no 2×. **R1 arranca de los MISMOS valores `{70,70,107}` (decisión Gustavo 2026-06-09) — A-VERIFICAR-EN-BANCO**: su `{70,70,42}` previo era del banco R1 2026-06-08 (la trasera se bajó porque el robot ROTABA en el strafe); si R1 rota con 107, bajar idx2 gradualmente. Acompañan al piso (estándar lateral): impulso inicial `{130,130,140}`×40 ms (`-DCENTRAL_MOTOR_KICKSTART`) + freno anticipado trasera 66 ms (`-DCENTRAL_REAR_BRAKE_LEAD`). |
| `MOTOR_PWM_NOISE_THRESH` | `config_central.h:133` | 5 (global) | **HARDCODED-COMÚN** | ninguno | filtra el jitter del comando para no zumbar parado; global a ambos. |
...
| Clock I²C | `sensors_imu.cpp:188` `Wire.setClock(100000)` (baseline) + `sensors_tof.cpp` brackea a `TOF_FAST_BUS_HZ` | base 100 kHz; el read de ToF a **400 kHz** (`-DTOP_TOF_FAST_BUS`) | **HARDCODED-COMÚN** (el flag está en `top_robot1_pri`/`top_robot2_pri`) | ninguno | ✅ HECHO 2026-06-22 (R1+R2): con el primario en `Wire2`, el bus de ToF sube a 400 SOLO durante `getRangingData()`; baseline + centinela siguen a 100. |
...
- **HARDCODED-COMÚN candidatos a per-robot si difieren chasis:** `WHEEL_ANGLES_DEG` (ya CALIBRADO 2026-06-08 para R1, común a ambos hoy), `WHEEL_RADIUS_MM`, `TOF_OFFSET_MM`, `OTOS_SEPARATION_MM`, `HEADING_SIGN`. **≈ 5.**
