...
| `MOTOR_MIN_PWM[3]` | `config_central.h:53` (R1) / `:78` (R2) | R1 `{70, 70, 42}` · R2 `{0, 0, 0}` (neutro) | **PER-ROBOT (HW)** | `#if ROBOT1/ROBOT2` | Piso de PWM **POR RUEDA** (reemplazó el viejo escalar único y el efímero `MOTOR_GAIN`). La fricción que ve cada rueda es distinta: delanteras OBLICUAS (60° → más fricción del rodillo → piso 70) > trasera PARALELA al strafe (menos fricción → piso 42). El PWM **NO** es proporcional a la velocidad de rueda. R2 en `{0,0,0}` hasta calibrar (pines rotados → idx≠rueda). |
