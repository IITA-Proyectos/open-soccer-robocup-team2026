...
| **Motores ROBOT2 — pines/inversión dudosos** | `config_central.h` `#if ROBOT2` con `MOTOR_INVERT` copiado de R1. | El perfil lleva `PIN_INA/INB/PWM` por motor + `MOTOR_INVERT[]`. **Validar con `diag_central_motors` en R2** y fijar los reales (el usuario advierte posible error de armado: pines o dirección). (ROBOT1 ya está validado en banco: M1=U5, M2=U17, M3=U7; con el cableado de fábrica el M2/U17 estaba invertido HW → `{+1,-1,+1}`, pero desde el recableado de la reparación 2026-06-10/11 quedó DERECHO → `MOTOR_INVERT={+1,+1,+1}` validado en piso 2026-06-11, commit 8d5fc90). El 'dudoso' aplica solo al delantero/ROBOT2. |
