# main-comunicacion-vieja.py — Cámara Soccer 2026 (OpenMV) — VERSIÓN QUE ANDA, COMUNICACIÓN VIEJA (INSEGURA)
#
# ⚠️ ESTE NO ES EL DE PRODUCCIÓN. El de producción es `main.py` (misma detección,
#    comunicación SEGURA contrato v2 que el TOP parsea).
#
# Para qué se guarda
# ------------------
# Es el script que FUNCIONA en banco para detectar (calibración LAB real, LEDs,
# homografía de Elías 2026-06-07). Se conserva TAL CUAL para PRUEBAS / referencia
# de la detección. Su ÚNICO problema es la comunicación: manda 9 bytes con X SIN
# codificar (puede ser negativo -> rompe bytearray), "no detectado" = 0/0, y SIN
# CRC ni byte de fin -> el TOP NO la parsea de forma confiable.
#
# El `main.py` de esta misma carpeta es este mismo script con SOLO la comunicación
# cambiada al formato seguro de 11 bytes. Usar ESE en la cámara para que el TOP
# reciba bien. Este queda para comparar / debuggear la detección si hiciera falta.
...
while(True):
    ...
    azul_blobs = img.find_blobs([azul_threshold], pixels_threshold=600, area_threshold=600, merge=True)
