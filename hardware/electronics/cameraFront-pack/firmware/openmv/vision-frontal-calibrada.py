# vision-frontal-calibrada.py — ARTEFACTO DE BANCO (calibración de distancias)
# ─────────────────────────────────────────────────────────────────────────────
# Autor de la calibración: Elías · Fecha: 2026-06-07 · Robot: ROBOT1 (arquero)
# Cámara: FRONTAL · Lente: ULTRA-WIDE · Resolución: VGA (640×480, "intermedia")
#
# QUÉ ES: el script con el que Elías calibró en banco la HOMOGRAFÍA de distancias
# de la cámara frontal del ROBOT1. La matriz `H` de `transformarcoordenadas()` +
# `h` (altura cámara) + los thresholds LAB son el RESULTADO de esa calibración.
#
# ⚠️ IMPORTANTE — por qué este archivo NO se flashea tal cual en competencia:
#   1) PROTOCOLO VIEJO (v1): manda 9 bytes [201,X,Y+100, 202,…, 203,…] SIN CRC ni
#      END ni sentinel, y X va crudo (no simétrico). El firmware del TOP ya está
#      en CONTRATO v2 (11 bytes, X+100 simétrico, CRC8 + END=254, sentinel 255).
#      Flashear esto rompe la cadena cámara→TOP. → La H calibrada se PORTÓ a
#      `cam-frontal-n6.py` (v2) que es el que SÍ se flashea.
#   2) Las distancias (X,Y en cm) están tomadas desde el CENTRO DEL LENTE, NO
#      desde el centro del robot. Hay que restar el offset lente→centro aguas
#      abajo (ver docs/firmware/CALIBRACION-HOMOGRAFIA-XY-N6.md).
#
# Fuente única de la calibración portada: docs/firmware/CALIBRACION-HOMOGRAFIA-XY-N6.md
# y H_MATRIX en cam-frontal-n6.py. Este archivo se conserva como PRUEBA del banco.
# ─────────────────────────────────────────────────────────────────────────────
