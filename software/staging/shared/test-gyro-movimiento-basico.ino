...
//
// LECCIONES APRENDIDAS (ver docs/internal/lecciones-pid-movimiento.md):
//   - La corrección PID debe INVERTIRSE cuando el robot va hacia atrás
//   - Usar detección de flanco + esperarSoltarBoton() para evitar rebotes
//   - bno.begin() sin parámetros es más compatible que OPERATION_MODE_IMUPLUS
//   - NO depender de zirconLib para tests simples - definir pines directamente
//
// BASADO EN: definitivo-delantero (ROBOT2)
// NO USA zirconLib - pines definidos directamente
//
// Author: Claude (Anthropic)
// Requested-by: María Virginia Viollaz
// =============================================================================
...
// Corrección PID NORMAL: M1 += correccion, M2 -= correccion
// =============================================================================
...
// =============================================================================
// FUNCIÓN: moverAtras()
// Corrección PID INVERTIDA: M1 -= correccion, M2 += correccion
// IMPORTANTE: Al ir hacia atrás, la corrección debe invertirse porque
// el "frente" del movimiento es la parte trasera del robot.
// =============================================================================
void moverAtras(int velocidadBase) {
  float heading = leerHeading();
  float correccion = calcularCorreccionPID(heading);
  
  // ¡INVERTIDO respecto a moverAdelante()!
  int velM1 = velocidadBase - (int)correccion;
  int velM2 = velocidadBase + (int)correccion;
...
// Detecta FLANCO DE SUBIDA del botón (evita múltiples detecciones)
// =============================================================================
...
// Espera a que el usuario SUELTE el botón antes de continuar
// =============================================================================
...
void imprimirMenu() {
  ...
  switch (estadoTest) {
    ...
    case ESPERANDO_LOOP:
      Serial.println("PRESIONAR BOTON para iniciar LOOP");
      Serial.println("(Adelante 5s -> Atras 5s -> repetir)");
      break;
...
void setup() {
  ...
  Serial.println("*  Tiempo: 5 segundos cada direccion               *");
  Serial.println("*  Modo: LOOP INFINITO (adelante/atras)            *");
  Serial.println("****************************************************");
  ...
  if (!bnoOK) {
    Serial.println("\n*** ERROR: Giroscopo no inicializado ***");
    while (true) {
      digitalWrite(LED_BUILTIN, HIGH);
      delay(100);
      digitalWrite(LED_BUILTIN, LOW);
...
void loop() {
  ...
  switch (estadoTest) {
    ...
    case LOOP_PAUSA_1:
      parar();
      digitalWrite(LED_BUILTIN, (millis() / 100) % 2);  // Parpadeo rapido = pausa
    ...
    case LOOP_ATRAS:
      ...
      digitalWrite(LED_BUILTIN, LOW);  // LED apagado = atras
