// main_centralconduce.cpp — entrypoint del programa AUTOCONTENIDO "centralconduce".
//
// NO es el rodeo de Edge. Estrategia "CONDUCIR la pelota al arco": busca la pelota, la CENTRA al
// frente, la LLEVA al arco (o al heading 0 si no ve el arco), y a <40 cm del arco APUNTA y PATEA.
// Lógica en mix_fsm_edge.cpp. (El rodeo estilo Edge vive en `centraledge`/`centraledgefijo`.)
// centralmix queda PRISTINO. El env central_robot1_mix_conduce compila SOLO esta carpeta
// (build_src_filter = +<centralconduce/> +<shared/>) con -DMIX_ATTACK_EDGE.
//
