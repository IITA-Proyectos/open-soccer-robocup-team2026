...
// ============================================================
// CONDUCIR la pelota al arco (centralconduce) — coach + Elías, 2026-06-23.
// Solo se usa con -DMIX_ATTACK_EDGE (FSM mix_fsm_edge.cpp). Sin el flag, centralmix
// corre EXACTAMENTE como hoy (FSM 2025 en mix_fsm.cpp): 100% aditivo.
//
// QUÉ ES (NO es el rodeo de Edge): busca la pelota → la CENTRA al frente (para que al
// avanzar no se le escape) → la LLEVA al arco (o al heading 0 si no ve el arco), manteniéndola
// al frente → cuando está a <40 cm del arco, APUNTA al arco y PATEA (empuje por inercia).
// NO se posiciona exactamente detrás de la pelota respecto al arco: solo la centra y la lleva.
//
// ⚠️ NADA testeado en hardware. Toda PERILLA se titula en banco. Unidades de mix_io
// (cm CRUDO de cámara, grados marco robot; goal_opp_dist en mm).
// ============================================================

// --- Velocidad de avance (PWM de traslación) mientras lleva la pelota. ---
constexpr int   MIX_EDGE_SPEED     = 180;     // PWM de avance llevando la pelota. Rango 140..220.

// --- Giro para ORIENTAR el frente (al arco, al heading 0, o a la pelota al centrar). ---
constexpr float MIX_EDGE_FACE_KP   = 1.5f;    // PWM de giro por GRADO de error de orientación.
                                              //   *** PERILLA DE BANCO (signo Y magnitud) ***.
                                              //   Confirmar SIGNO con robot levantado (si gira al
                                              //   revés → -1.5). Después subir magnitud sin zigzag.
constexpr int   MIX_EDGE_OMEGA_MAX = 70;      // tope del PWM de giro (no dominar al avance).

// --- "Ver la pelota al frente" (centrar): tolerancias de ángulo a la pelota. ---
constexpr float MIX_CONDUCE_CENTER_TOL_DEG    = 12.0f; // |ang pelota| < esto = está al FRENTE (centrada).
constexpr float MIX_CONDUCE_RECAPTURE_TOL_DEG = 35.0f; // si llevándola |ang pelota| > esto = se fue al
                                                       //   costado → volver a CENTRAR. (>CENTER, con histéresis.)
constexpr float MIX_CONDUCE_HAVE_DIST_CM      = 20.0f; // pelota más cerca que esto = "ya la tengo" →
                                                       //   recién ahí escolto hacia el arco (antes solo la sigo).

// --- Llevar al arco / apuntar y patear. ---
constexpr float MIX_CONDUCE_KICK_GOAL_DIST_CM = 40.0f; // a MENOS de esto del arco → apuntar y patear.
                                                       //   (goal_opp_dist viene en mm; se compara /10.)
constexpr float MIX_CONDUCE_AIM_TOL_DEG       = 12.0f; // arco dentro de esto al frente = apuntado → patear.
...
// --- Pérdida de pelota → volver a buscar. ---
