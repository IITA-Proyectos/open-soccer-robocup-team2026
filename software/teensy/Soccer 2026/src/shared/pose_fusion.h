...
struct PoseFusionConfig {
    ...
    // POSE-02 (eval 2026-06-04) / anti-free-run (2026-06-16): otos_stale_ms HOY no se
    // consume acá — la freshness del OTOS la gatea el CALLER vía in.otos_fresh antes de
    // entrar (ver contrato en PoseFusionInputs). tof_stale_ms SÍ se consume desde el fix
    // anti-free-run: gatea la EXPIRACIÓN de la salida (ver pose_fusion_update PASO 4.5).
    uint16_t otos_stale_ms;           // [no leído aún] si dt desde último OTOS fresco > esto => sin predicción. Default 60.
    uint16_t tof_stale_ms;            // CONSUMIDO: si ms_since_tof_corr > esto => salida invalidada (valid=false, conf=0). Default 500.
    ...
    uint8_t  conf_min;                // piso de confidence MIENTRAS NO esté vencido por edad
                                      // (ms_since_tof_corr <= tof_stale_ms). Pasado el corte, la
                                      // salida se invalida (conf=0), NO se sostiene en este piso. Default 10.
