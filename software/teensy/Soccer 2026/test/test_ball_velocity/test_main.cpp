...
void test_first_visible_sample_not_valid(void) {
    ball_velocity_update(s, P, 100, 50, /*visible=*/true, /*sample_ms=*/1000);
...
void test_two_samples_x_velocity(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);   // +100 mm en 100 ms → 1000 mm/s
...
void test_two_samples_y_velocity(void) {
    ball_velocity_update(s, P, 0, 0,   true, 0);
    ball_velocity_update(s, P, 0, 200, true, 100);   // +200 mm en 100 ms → 2000 mm/s
    TEST_ASSERT_EQUAL_INT16(0,    ball_velocity_vx_mm_s(s));
...
void test_negative_velocity(void) {
    ball_velocity_update(s, P, 100, 0, true, 0);
    ball_velocity_update(s, P, 0,   0, true, 100);   // -100 mm en 100 ms → -1000 mm/s
    TEST_ASSERT_EQUAL_INT16(-1000, ball_velocity_vx_mm_s(s));
}

// --- pelota quieta: válida pero v=0 (distinto de "no la veo") --------------
void test_stationary_is_valid_zero(void) {
    ball_velocity_update(s, P, 50, 50, true, 0);
    ball_velocity_update(s, P, 50, 50, true, 100);
    TEST_ASSERT_TRUE(s.valid);
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vx_mm_s(s));
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vy_mm_s(s));
}
...
void test_ema_blends_second_derivative(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);   // siembra vx=1000
    ball_velocity_update(s, P, 300, 0, true, 200);   // inst=2000; EMA .5 → 1500
...
void test_not_visible_resets(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);   // vx=1000, valid
    ball_velocity_update(s, P, 0,   0, /*visible=*/false, 200);
    TEST_ASSERT_FALSE(s.valid);
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vx_mm_s(s));
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vy_mm_s(s));
}
...
void test_reappear_no_spike(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);   // vx=1000
    ball_velocity_update(s, P, 0,   0, false, 200);  // pierde
    ball_velocity_update(s, P, 5000, 0, true, 300);  // reaparece LEJOS → no spike
    TEST_ASSERT_FALSE(s.valid);
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vx_mm_s(s));
    ball_velocity_update(s, P, 5100, 0, true, 400);  // recién acá vuelve a derivar
    TEST_ASSERT_TRUE(s.valid);
    TEST_ASSERT_EQUAL_INT16(1000, ball_velocity_vx_mm_s(s));
}
...
void test_no_new_sample_is_noop(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);          // vx=1000
    ball_velocity_update(s, P, 999, 999, true, 100);        // mismo sample_ms → ignorar
    TEST_ASSERT_TRUE(s.valid);
    TEST_ASSERT_EQUAL_INT16(1000, ball_velocity_vx_mm_s(s));
}
...
void test_large_gap_reseeds(void) {
    ball_velocity_update(s, P, 0,   0, true, 0);
    ball_velocity_update(s, P, 100, 0, true, 100);   // vx=1000, valid
    ball_velocity_update(s, P, 500, 0, true, 400);   // dt=300 > 200 → re-siembra
    TEST_ASSERT_FALSE(s.valid);
    TEST_ASSERT_EQUAL_INT16(0, ball_velocity_vx_mm_s(s));
    ball_velocity_update(s, P, 600, 0, true, 450);   // dt=50 → 2000 mm/s
    TEST_ASSERT_TRUE(s.valid);
...
void test_getter_clamps_positive(void) {
    ball_velocity_update(s, P, 0,     0, true, 0);
    ball_velocity_update(s, P, 30000, 0, true, 1);   // 3e7 mm/s → satura
    TEST_ASSERT_EQUAL_INT16(32767, ball_velocity_vx_mm_s(s));
}
void test_getter_clamps_negative(void) {
    ball_velocity_update(s, P, 0,      0, true, 0);
    ball_velocity_update(s, P, -30000, 0, true, 1);
